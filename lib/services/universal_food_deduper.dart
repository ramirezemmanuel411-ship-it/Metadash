...
class UniversalFoodDeduper {
  ...
  static String _extractDietType(String text) {
    ...
    if (text.contains('zero sugar') || text.contains('zero cal')) {
      return 'zero';
    }
    if (text.contains('zero') ||
        text.contains('zéro') ||
        text.contains('0 sugar')) {
      return 'zero';
    }
    if (text.contains('sugar free') || text.contains('sugarfree')) {
      return 'sugar-free';
    }
    if (text.contains('light') || text.contains('lite')) {
      return 'light';
    }
    if (text.contains('low calorie')) {
      return 'low-cal';
    }
    return 'regular';
  }
  ...
  static String _extractFormat(String text) {
    if (text.contains('mini can') || text.contains('sleek can')) {
      return 'mini-can';
    }
    ...
    if (text.contains('chips') || text.contains('crisps')) {
      return 'chips';
    }
    if (text.contains('snack pack')) {
      return 'snack-pack';
    }
    return '';
  }
  ...
  static String _extractFatLevel(String text) {
    if (text.contains('nonfat') ||
        text.contains('non fat') ||
        text.contains('fat free')) {
      return 'nonfat';
    }
  ...
  static String inferCoreName(
    ...
    if (core.isEmpty) {
      ...
      if (queryNorm != null && queryNorm.contains('coke')) {
        return 'coke';
      }
      if (queryNorm != null && queryNorm.contains('cola')) {
        return 'cola';
      }
  ...
  static String normalizeBrand(String? brandRaw, String? nameNorm) {
    ...
    if (brandRaw == null || brandRaw.isEmpty) {
      ...
      if (nameNorm.contains('pepsi')) {
        return 'pepsi';
      }
      if (nameNorm.contains('sprite')) {
        return 'sprite';
      }
      if (nameNorm.contains('fanta')) {
        return 'fanta';
      }
      return 'generic';
    }
  ...
  static String buildFamilyKey({
    ...
    // Build composite key
    final parts = [brandNorm, coreNorm, variants.dietType, variants.flavor];
    ...
    if (debug) {
      print('   [KEY] "$name"');
      print(
        '      nameNorm="$nameNorm" brandNorm="$brandNorm" coreNorm="$coreNorm"',
      );
  ...
  static DeduplicationResult deduplicateByFamily({
    ...
    final itemSignatures =
        <String, String>{}; // map item.id -> family signature
    final itemDetails =
        <String, Map<String, String>>{}; // map item.id -> details for printing
    ...
    print(
      '   📊 Grouped into ${familyGroups.length} families (before second pass)',
    );
    ...
    for (final entry in familyGroups.entries) {
      ...
      if (candidates.length > 1) {
        ...
        print(
          '      • ${candidates.length} candidates → selected "${best.displayTitle}"',
        );
        final collapsed = candidates
            .where((c) => c.id != best.id)
    ...
    print(
      '   📤 Output: ${representatives.length} items (collapsed $collapsedCount duplicates)',
    );
    ...
    if (debug) {
      ...
      for (int i = 0; i < filtered.length && i < 15; i++) {
        ...
        print(
          '        nameNorm="${details['nameNorm']}" brandNorm="${details['brandNorm']}"',
        );
        print(
          '        coreName="${details['coreName']}" diet="${details['dietType']}" flavor="${details['flavor']}"',
        );
  ...
  static double jaroWinklerSimilarity(String s1, String s2) {
    ...
    final matchWindow =
        (({len1, len2}.reduce((a, b) => a > b ? a : b) / 2).ceil() - 1)
  ...
  static List<FoodModel> _secondPassDedup(List<FoodModel> items, String query) {
    ...
    for (final item in items) {
      ...
      for (final other in items) {
        ...
        if (variants1.dietType == variants2.dietType &&
          ...
          print(
            '      [MERGED] "${item.displayTitle}" + "${other.displayTitle}"',
          );
        }
      }
    ...
    return merged
        .map(
          (group) => _selectBestRepresentative(
            candidates: group,
            query: query,
            debug: false,
          ),
  ...
  static List<FoodModel> _applyQueryRelevance(
