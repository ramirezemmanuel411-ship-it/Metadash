...
// ─────────────────────────────────────────────────────────────────────────────
// Serving Section — quantity field + unit selector
// ─────────────────────────────────────────────────────────────────────────────
...
class _UnitOption {
  ...
  const _UnitOption.weight(this.label, this.gramsPerUnit)
      : isWeight = true, servingIdx = null;
  const _UnitOption.named(this.label, this.servingIdx)
      : isWeight = false, gramsPerUnit = null;
}

class _ServingSection extends StatefulWidget {
  final FoodModel food;
  final List<({String label, double? grams})> servings;
  final void Function(double quantity, int servingIdx, String label) onChanged;

  const _ServingSection({
    required this.food,
  ...
  @override
  State<_ServingSection> createState() => _ServingSectionState();
}

class _ServingSectionState extends State<_ServingSection> {
  final TextEditingController _ctrl = TextEditingController();
  final FocusNode _focusNode = FocusNode();

  late List<_UnitOption> _units;
  int _unitIdx = 0;
  ...
  void _buildUnits() {
    ...
    final hasG = defG > 0;
    if (hasG) {
      _units.add(const _UnitOption.weight('g',  1.0));
      _units.add(const _UnitOption.weight('oz', 28.3495));
      _units.add(const _UnitOption.weight('lb', 453.592));
      _units.add(const _UnitOption.weight('ml', 1.0));
    }
    final seen = <String>{};
    for (int i = 0; i < widget.servings.length; i++) {
      // Strip "(Xg)" / "(X g)" annotations — they cause "1 | 100 g" confusion
      var lbl = widget.servings[i].label
          .replaceAll(RegExp(r'\s*\(\d+\.?\d*\s*g\)\s*$'), '')
          .trim();
      if (lbl.isEmpty || seen.contains(lbl)) continue;
      seen.add(lbl);
      _units.add(_UnitOption.named(lbl, i));
    }
    if (_units.isEmpty) _units.add(const _UnitOption.named('serving', 0));
  }

  void _setDefaultUnit() {
    final base = _baseGrams ?? 0;
    if (base > 0) {
      _unitIdx = 0; // grams
      _ctrl.text = _fmt(base);
    } else {
      _unitIdx = _units.indexWhere((u) => !u.isWeight);
      if (_unitIdx < 0) _unitIdx = 0;
      _ctrl.text = '1';
    }
  }
  ...
  String _fmt(double v) {
    ...
    final dec = (_units.isNotEmpty && _unitIdx < _units.length && _units[_unitIdx].isWeight) ? 1 : 2;
    return v.toStringAsFixed(dec).replaceAll(RegExp(r'\.?0+$'), '');
  }

  double? _toGrams(double amt) {
    if (_unitIdx >= _units.length) return null;
  ...
  double _fromGrams(double grams, _UnitOption u) {
  ...
  void _selectUnit(int idx) {
    ...
    final grams = _toGrams(double.tryParse(_ctrl.text) ?? 1.0);
    final newAmt = grams != null
        ? _fromGrams(grams, _units[idx]).clamp(0.001, 99999.0)
        : 1.0;
    setState(() {
      _unitIdx = idx;
      _ctrl.text = _fmt(newAmt);
    });
    _notify();
  }
  ...
  void _notify() {
    ...
    final u = _units[_unitIdx];
    final amt = double.tryParse(_ctrl.text.trim()) ?? 1.0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Quantity field + unit selector in one row
          Container(
            height: 60,
            decoration: BoxDecoration(
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (raw) {
                      final v = double.tryParse(raw.trim());
                      if (v != null && v > 0) _notify();
                    },
                    onSubmitted: _commit,
                  ),
                ),
                Container(
                    width: 1, height: 30, color: accent.withValues(alpha: 0.2)),
                GestureDetector(
    ...
    showModalBottomSheet<void>(
      ...
      builder: (sheetCtx) {
        return ListView.builder(
          ...
          itemBuilder: (_, i) {
            final sel = i == _unitIdx;
            return ListTile(
              title: Text(_units[i].label),
              trailing: sel
