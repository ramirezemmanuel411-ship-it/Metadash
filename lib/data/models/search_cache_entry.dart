...
class SearchCacheEntry extends Equatable {
  ...
  /// Bump when any food-name parsing change is deployed so old cached
  /// entries (with corporate names) are not returned to the user.
  ...
  static String createKey(String query, {Map<String, dynamic>? filters}) {
    ...
    if (filters == null || filters.isEmpty) {
      return '$_cacheVersion:$normalized';
    }
    final filterStr = filters.entries
        .map((e) => '${e.key}:${e.value}')
        .join('|');
    return '$_cacheVersion:$normalized#$filterStr';
  }
  ...
  factory SearchCacheEntry.fromJson(Map<String, dynamic> json) {
    ...
    return SearchCacheEntry(
      ...
      results: results,
      timestamp: DateTime.fromMillisecondsSinceEpoch(json['updated_at'] as int),
