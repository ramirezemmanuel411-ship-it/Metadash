  ...
  group('Food Search Pipeline Integration Tests', () {
    ...
    test('Query "coke" returns correct Coca-Cola variants in order', () {
      ...
      final resultNames = results
          .map((r) => r.displayTitle.toLowerCase())
          .join(' ');
      expect(resultNames, anyOf(contains('diet'), contains('zero')));
      expect(
        resultNames,
        anyOf(contains('cherry'), contains('original'), contains('coke')),
      );

      // "Coca cola Gout Original" and "Coca Cola Coke Brand" should be deduplicated
      final goutCount = results
          .where((r) => r.name.contains('Gout') || r.name.contains('Go'))
          .length;
      expect(
        goutCount,
        lessThanOrEqualTo(2),
        reason: 'Foreign variants should be reduced',
      );
    ...
    test('Query "pepsi" returns multiple Pepsi variants (not just 1)', () {
      ...
      expect(
        ...
        reason: 'Should keep distinct variants',
      );
      ...
      expect(
        ...
        reason: 'Should not collapse all Pepsi into 1',
      );
    ...
    test('Query "pizza hut" returns Pizza Hut items first, NOT Lay\'s chips', () {
      ...
      for (var i = 0; i < top3.length; i++) {
        ...
        expect(
          ...
          isTrue,
          reason:
              'Top ${i + 1} result should be Pizza Hut branded, got: ${item.displayTitle}',
        );
      }
    ...
    test('Query "hershey" returns distinct products with clean names', () {
      final rawResults = [
        ...
        _createFood(
          id: '6',
          name: 'HERSHEY CHOCOLATE BAR 100G',
          brand: 'HERSHEY',
          calories: 540,
          servingSize: 100,
          servingUnit: 'g',
      ...
      expect(
        ...
        reason: 'Should keep distinct products',
      );
      expect(results.length, lessThanOrEqualTo(7));
      ...
      final foreignCount = results
          .where((r) => r.name.contains('chocolat au lait'))
          .length;
      expect(
        foreignCount,
        lessThanOrEqualTo(1),
        reason:
            'Foreign variants should be reduced when English equivalent exists',
      );

      print(
        '✓ Test passed: Hershey returns distinct products with clean names\n',
      );
    });
    ...
    test('Deduplication keeps "Coke Zero" and "Diet Coke" separate', () {
      final rawResults = [
        ...
        _createFood(
          ...
          servingSize: 355,
          servingUnit: 'ml',
        ), // Slightly different to avoid canonicalKey collision
        _createFood(
          id: '5',
          name: 'Diet Coke (100ml)',
          brand: 'Coca Cola',
          calories: 0,
          servingSize: 100,
          servingUnit: 'ml',
      ...
      expect(
        originalCount,
        lessThanOrEqualTo(2),
        reason: 'Should collapse duplicate originals',
      );

      // Should collapse "Diet Coke" and "Diet Coke (100ml)" into one
      final dietCount = results
          .where((r) => r.displayTitle.toLowerCase().contains('diet'))
          .length;
      expect(
        dietCount,
        lessThanOrEqualTo(2),
        reason: 'Should collapse serving size duplicates',
      );

      // Should keep Diet Coke and Coke Zero as separate (meaningful variants)
      final hasDiet = results.any(
        (r) => r.displayTitle.toLowerCase().contains('diet'),
      );
      final hasZero = results.any(
        (r) => r.displayTitle.toLowerCase().contains('zero'),
      );
      expect(
        hasDiet && hasZero,
        isTrue,
        reason: 'Should keep Diet and Zero as separate variants',
      );
    ...
    test('Display titles are clean and readable', () {
      final rawResults = [
        ...
        _createFood(
          id: '2',
          name: 'pepsi, cola-flavored beverage',
          brand: 'PepsiCo',
          calories: 150,
          servingSize: 355,
          servingUnit: 'ml',
...
void _printResults(List<FoodModel> results) {
  ...
  for (var i = 0; i < results.length && i < 10; i++) {
    ...
    print(
      '       Brand: ${item.displayBrand} | ${item.calories} cal | ${item.servingLine}',
    );
